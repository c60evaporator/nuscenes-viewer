...
async def import_map(
    ...
    logger.info("[%s] Schema validated (%.1fs)", location, time.perf_counter() - t0)
    ...
    async with AsyncSessionLocal() as db:
        try:
            ...
            await db.commit()
            logger.info("[%s] Map import complete. (total=%.1fs)", location, time.perf_counter() - t0)
...
async def import_all_maps(data_root: str = settings.NUSCENES_DATAROOT) -> None:
    ...
    logger.info("=== Map expansion import start (%d locations) ===", len(locations))
    for loc in locations:
        await import_map(loc, data_root)
    logger.info("=== Map expansion import complete. (total=%.1fs) ===", time.perf_counter() - t_all)
