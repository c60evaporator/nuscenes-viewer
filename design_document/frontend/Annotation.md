...
アノテーション機能の仕様をこちらに記載。nuScenes本体（トラッキング用オブジェクト）とMap expansion（2D HDマップ）2種類のアノテーションが存在

1. Annotation画面：nuScenes本体のバウンディングボックスアノテーションの編集・追加
2. Sample&Map画面：Map expansionアノテーションの編集・追加（ポリゴン、ライン、信号機）

の2種類のアノテーションを編集・追加する画面
...
- 左ペイン：表示・追加・編集対象のアノテーション（Scene, Sample, Instance）を選択
- 中央ペイン：GUIによるアノテーション表示と編集
- 右ペイン：アノテーションの情報表示およびボタン・テキストボックス・プルダウンによる編集

#### 左ペイン

- 上部にScene、Sample, Instance, Sensorフィルタを配置。以下のように動作
  - Sceneフィルタ
    - Sceneフィルタを変更すると、Scene選択が変更される
    - デフォルトでは名前順で最初のSceneが選択されている
  - Sampleフィルタ
    - Sampleフィルタを変更すると、Sample選択が変更される
    - Instance画面から「Annotations」ボタンで遷移した場合、このフィルタは常に無効化されている（"All"で固定）
    - Sample画面から「Annotations」ボタンで遷移した場合、このフィルタはSample画面で選択されていたSampleに固定される
    - デフォルトでは"All"が選択されている（何も選択されていないことに相当）
  - Instanceフィルタ
    - Instanceフィルタを変更すると、Instance選択が変更される（ただし、このフィルタは常に固定されているので手動で変更することはない）
    - Instance画面から「Annotations」ボタンで遷移した場合のみ、このフィルタは有効化される（それ以外の場合"All"で固定）
    - Instance画面から「Annotations」ボタンで遷移した場合、このフィルタはInstance画面で選択されていたInstanceに固定される
  - Sensorフィルタ
    - Sensorフィルタを変更すると、中央ペイン左上のカメラ画像が選択したセンサーのものに変更される
    - SensorフィルタはSampleが選択されている場合（Sampleフィルタ選択時、またはInstance固定モードでSampleリスト選択時）のみ有効化され、どちらも選択されていない場合は無効化
    - アノテーション選択が変更されたとき（中央ペインのバウンディングボックス、Sample固定モード時のInstanceリスト、Instance固定モード時のSampleリストいずれかをクリックしたときが該当）、自動で最も映りの良いカメラが選ばれ、Sensorフィルタの選択がこれに変更され、中央ペイン左上のカメラ画像もこれに従い変更される
    - アノテーションの編集・削除モード時にはSensorフィルタは手動操作不可となり、バウンディングボックスの位置に応じて最も映りの良いカメラにSensorフィルタおよび中央ペイン左上のカメラ画像が自動更新される
    - Sampleフィルタが選択されており、アノテーションが選択されていない場合、CAM_FRONTがデフォルト選択される
- 中央部にSampleまたはInstanceリストを配置。以下の情報が表示される
  - Instance画面から「Annotations」ボタンで遷移した場合、選択されているInstanceに含まれるアノテーションのSample一覧がリスト表示される
  - それ以外の場合、「Sample」フィルタで選択したSampleに含まれるアノテーションのInstance一覧がリスト表示される
  - リストを選択すると、当該アノテーションが選択される。選択に応じて右ペインのアノテーション情報や、左ペイン下部の「Edit BBox」ボタンの対象が更新される
- 下部にアノテーションの編集・追加・削除モードに遷移するボタンを配置。詳細は後述
  - デフォルト状態では「Edit BBox」「Delete BBox」「Add BBox」ボタンを配置。
  - Instance画面から「Annotations」ボタンで遷移した場合、「Add BBox」ボタンが「Add BBox to prev」「Add BBox to next」「Skip and Add BBox to prev」「Skip and Add BBox to next」ボタンに変わることもある。

#### 中央ペイン

- 4分割され、左上にカメラ画像、右上にLIDAR_TOP（BEV形式のLiDAR点群表示）、右下にEGO POSE、右下に3D VIEW（Three.jsによるLiDAR点群表示）
- EGO POSE以外のセンサデータは、上にアノテーション位置を表すバウンディングボックスを重ねて表示
- バウンディングボックスをクリックすると、当該アノテーションが選択される。選択に応じて右ペインのアノテーション情報や、左ペイン下部の「Edit BBox」ボタンの対象が更新される
- 左上のカメラ画像の表示対象カメラは左ペインのSensorフィルタの選択に応じて変わる
- 右下の3D VIEWには"Default"、"Front"、"Side"ボタンがあり、それぞれ以下のように視点が変更される
  - Defaultボタン：斜め上後方から見たデフォルト視点
  - Frontボタン：アノテーション編集・追加モード時のみ有効。対象バウンディングボックス前方から見た視点
  - Sideボタン：アノテーション編集・追加モード時のみ有効。対象バウンディングボックス左側面から見た視点

#### 左ペイン

- 上部にBounding box ctrl、中央にアノテーションのメタデータ表示、下部に編集結果反映ボタンを設置
- Bounding box ctrl：アノテーション編集・追加モード時のみ有効化。アノテーションの位置・方向・サイズを変更するボタン群
  - 回転ボタン：左回転と右回転ボタン（計2個）
  - 並進ボタン：上下左右それぞれのボタン（計4個）
  - サイズボタン：W,L,Hの拡大・縮小ボタン（計6個）
- アノテーションのメタデータ表示：アノテーションの各種情報を表示する
  - translation：アノテーションの位置（x,y,z）。アノテーション編集・追加モード時は数値を変更してEnterを押すと位置を変更できる
  - size：アノテーションのサイズ（W,L,H）。アノテーション編集・追加モード時は数値を変更してEnterを押すとサイズを変更できる
  - rotation：アノテーションの方向オイラー角（yaw,pitch,roll）。アノテーション編集・追加モード時は数値を変更してEnterを押すと方向を変更できる
  - sample：当該アノテーションのSample（タイムスタンプで表示）
  - instance：当該アノテーションのinstance。アノテーション追加モード時はプルダウンで変更できる（選択可能なinstacneは後述）
  - cateogory：当該アノテーションのcategory。アノテーション追加モード時はプルダウンで変更できる（categoryテーブルに登録されたカテゴリが選択可能）
  - token：当該アノテーションのtoken
  - prev：当該アノテーションのprev（同一インスタンスの前のアノテーションのtoken）
  - next：当該アノテーションのnext（同一インスタンスの後のアノテーションのtoken）
  - lidar_pts：当該アノテーションのバウンディングボックス内に含まれるLiDAR点群数（動的に計算される）
  - radar_pts：当該アノテーションのバウンディングボックス内に含まれるRADAR点群数（動的に計算される）
- 編集結果反映ボタン：アノテーション編集・追加モード時のみ有効化。Save BBoxボタンとCancel Editボタンからなる
  - Save BBox：アノテーション編集結果を確定してDBに保存する
  - Cancel Edit：アノテーション編集結果を破棄してキャンセルする
...
##### 欠番を飛び越えたアノテーションの追加機能

上記の仕組みだけだと、遮蔽等でインスタンスが欠番となったフレーム（Sample）を飛び越えてアノテーションを登録することができない。よって以下のような「Add BBox to prev」と「Add BBox to next」を起点として欠番を飛び越えたアノテーションを追加する機能を実装する。

- Instanceフィルタ時のみ有効（Sampleフィルタ時は欠番フレーム飛び越えアノテーションは不要）
- 「Add BBox to next」ボタンで編集中、Scene内の最後のSampleでなければ「Skip and Add BBox to next」ボタンを有効化し、これを押すと次のSampleに遷移し、そのSampleとInstanceを選択した状態でのBBox編集モードに移る
  - 「Skip and Add BBox to next」ボタンで編集中、Scene内の最後のSampleでなければ「Skip and Add BBox to next」ボタンを同様に有効化し、押したら次のSampleに遷移する
  - 「Skip and Add BBox to next」ボタンで編集中、Save BBoxを押したしたときにPOSTされるprevは最初に「Add BBox to next」ボタンを押したときのSampleのアノテーションとなるよう適切にtokenを保持しておく
  - POSTした際に、prevに相当するsample_annotationのレコードのnextが追加したレコードのtokenとなるようバックエンド側で適切に更新する
- 同様に「Add BBox to prev」ボタンで編集中、Scene内の最初のSampleでなければ「Skip and Add BBox to prev」ボタンを有効化し、これを押すと前のSampleに遷移し、そのSampleとInstanceを選択した状態でのBBox編集モードに移る
  - 「Skip and Add BBox to prev」ボタンで編集中、Scene内の最初のSampleでなければ「Skip and Add BBox to prev」ボタンを同様に有効化し、押したら前のSampleに遷移する
  - 「Skip and Add BBox to prev」ボタンで編集中、Save BBoxを押したしたときにPOSTされるnextは最初に「Add BBox to prev」ボタンを押したときのSampleのアノテーションとなるよう適切にtokenを保持しておく
  - POSTした際に、nextに相当するsample_annotationのレコードのprevが追加したレコードのtokenとなるようバックエンド側で適切に更新する

##### 追加モード時のバウンディングボックス位置・方向・サイズのデフォルト値

追加（Add BBox）モード時のバウンディングボックスのtranslation、size、rotation入力を効率化するため、以下のルールに従いtranslation、size、rotationのデフォルト値を決める。

- 同一Instanceのアノテーションがある場合（Instanceフィルタ適用時、またはSampleフィルタ適用時に右ペインInstanceドロップダウンで既存Insatance選択時）：今回アノテーションを追加するSampleのタイムスタンプを$t_0$とし、以下の優先度に従いtranslation、size、rotationのデフォルト値を決定
  - 優先度1（線形内挿補完）：選択中Sampleより前に1個以上、かつ後ろに1個以上のアノテーションがある場合：選択中Sampleよりも前の最も近いSampleにおけるtranslation,size,rotationをベクトルで表したものとタイムスタンプを$\boldsymbol{t}_1,\boldsymbol{s}_1,\boldsymbol{r}_1,T_1$、選択中Sampleよりも後ろの最も近いSampleにおけるベクトルとタイムスタンプを$\boldsymbol{t}_2,\boldsymbol{s}_2,\boldsymbol{r}_2,T_2$としたとき、translationのデフォルト値を$\boldsymbol{t}_1+(\boldsymbol{t}_2-\boldsymbol{t}_1)*\frac{T_0-T_1}{T_2-T_1}$とする。sizeとrotationのデフォルト値は$∣T_0−T_1​∣<∣T_0​−T_2​∣$なら$\boldsymbol{s}_1,\boldsymbol{r}_1$​、そうでなければ$\boldsymbol{s}_2,\boldsymbol{r}_2$とする（近い方を採用するという意図）
  - 優先度2（線形外挿補完）：選択中Sampleよりも前のSampleのアノテーションが2個以上ある場合：選択中Sampleよりも前の最も近いSampleにおけるtranslation,size,rotationをベクトルで表したものとタイムスタンプを$\boldsymbol{t}_1,\boldsymbol{s}_1,\boldsymbol{r}_1,T_1$、もう一つ前のsampleのベクトルとタイムスタンプを$\boldsymbol{t}_2,\boldsymbol{s}_2,\boldsymbol{r}_2,T_2$とし、優先度1と同じ計算式でtranslationのデフォルト値を求め、sizeとrotationのデフォルト値を$\boldsymbol{s}_1,\boldsymbol{r}_1$とする
    - 時間ギャップが大きい場合の暴走リスクを防ぐため、$|T_0-T_1|$が`frontend/config/setting.yml`の`annotation.translation_extrapolation_max`を上回ったら線形外挿補完はせずtranslationのデフォルト値を$\boldsymbol{t}_1$とする（優先度3のケースでも同様）
  - 優先度3（線形外挿補完）：選択中Sampleよりも後のSampleのアノテーションが2個以上ある場合：選択中Sampleよりも後の最も近いSampleにおけるtranslation,size,rotationをベクトルで表したものとタイムスタンプを$\boldsymbol{t}_1,\boldsymbol{s}_1,\boldsymbol{r}_1,T_1$、もう一つ後のtranslation,size,rotationのベクトルとタイムスタンプを$\boldsymbol{t}_2,\boldsymbol{s}_2,\boldsymbol{r}_2,T_2$とし、優先度2と同じ計算式でデフォルト値を求める
  - 優先度4：選択中Sampleよりも前のSampleのアノテーションが1個ある場合：前のSampleのアノテーションのtranslation,size,rotationをデフォルト値として使用する
  - 優先度5：選択中Sampleよりも後のSampleのアノテーションが1個ある場合：後のSampleのアノテーションのtranslation,size,rotationをデフォルト値として使用する
- 同一Instanceのアノテーションがない場合
  - size：選択categoryと`frontend/config/setting.yml`の`annotation.default_bbox_sizes`に応じて決める（現状と同じ）
  - rotation：以下の式に基づき、egoのyawのみを抽出してz軸回転のクォータニオンを構成する
  - translation：x,yはegoから直進方向に見て`frontend/config/setting.yml`の`annotation.default_forward_distance`メートル前に設定。zは以下の地面高さ検出アルゴリズムで`(x,y)`に上で定めたx,yを指定して求めた値を用い、sizeを考慮して地面の高さとバウンディングボックスの底面の高さが一致するようにする

```
yaw = atan2(2(wz + xy), 1 - 2(y^2+ z^2))   # ego quaternion から yaw 抽出
rotation = [cos(yaw/2), 0, 0, sin(yaw/2)]
```
