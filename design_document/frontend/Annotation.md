...
- Instanceフィルタ適用時（Sampleフィルタは「すべて」）
  ...
  - Sampleを選択している場合：「Edit BBox」ボタンのみ有効。「Add BBox」は以下の場合を除き無効化
...
- Register the BBox：編集した情報をバックエンドに登録して編集・追加モードを抜ける
- Cancel the Edit：編集した情報をバックエンドに登録せずに編集・追加モードを抜ける
- 画面上側のタブ切り替え：Cancel the Editと同じ効果

編集・追加中は"Cancel the Edit"ボタンと画面上側のタブ切り替え以外で操作がキャンセルされないよう、以下の処理を無効化する
...
追加モードでは、バウンディングボックスの選択をいったん解除した上で、各情報を以下のように取得して表示
