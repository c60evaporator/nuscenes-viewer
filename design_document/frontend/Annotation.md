...
- 同一Instanceのアノテーションがある場合：今回アノテーションを追加するSampleのタイムスタンプを$t_0$とし、以下の優先度に従いtranslation、size、rotationのデフォルト値を決定
  - 優先度1：選択中Sampleよりも前のSampleのアノテーションが2個以上ある場合：選択中Sampleよりも前の最も近いSampleにおけるtranslation,size,rotationをベクトルで表したものとタイムスタンプを$/boldsymbol{p}_1,t_1$、もう一つ前のsampleのベクトルとタイムスタンプを$/boldsymbol{p}_2,t_2$としたとき、translation,size,rotationのデフォルト値を$/boldsymbol{p}_1+(/boldsymbol{p}_1-/boldsymbol{p}_2)*/frac{t_0-t_1}{t_1-t_2}$とする
  - 優先度2：選択中Sampleよりも後のSampleのアノテーションが2個以上ある場合：選択中Sampleよりも後の最も近いSampleにおけるtranslation,size,rotationをベクトルで表したものとタイムスタンプを$/boldsymbol{p}_1,t_1$、もう一つ後のsampleのベクトルとタイムスタンプを$/boldsymbol{p}_2,t_2$とし、優先度1と同じ計算式でデフォルト値を求める
  その中で選択中Sampleに近いSampleのアノテーションを参照先とする
  - 優先度3：選択中Sampleよりも前のSampleのアノテーションが1個ある場合：当該アノテーションのtranslation,size,rotationをデフォルト値として使用する
  - 優先度4：選択中Sampleよりも後のSampleのアノテーションが1個ある場合：当該アノテーションのtranslation,size,rotationをデフォルト値として使用する
- 同一Instance直前または直後のアノテーションがない場合
  - size：
