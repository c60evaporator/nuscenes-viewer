...
static_oac_id   = ""                    # 後で修正するので最初は空欄でOK
web_acl_id      = ""                    # 後で修正するので最初は空欄でOK
...
バックエンド更新用リソースは`terraform`フォルダ内に実装しています。
...
```terraform.tfvars
static_oac_id   = "{既存static用OAC ID}"
web_acl_id      = "{既存Web ACL ID}"
```

Web ACL IDは以下コマンドで調べられます

```zsh
aws cloudfront get-distribution \
  --id {既存Distribution ID} \
  --query "Distribution.DistributionConfig.WebACLId" \
  --output text \
  --region us-east-1
```

以下コマンドで既存Distribution IDをimportします（CloudFront → DistributionsのID列に記載されている）
...
あとは以下コマンドでリソースをまとめてデプロイできます

```zsh
AWS_PROFILE=terraform terraform apply
```

##### 【参考】既存の無料ディストリビューション削除後のデプロイ方法

既存の無料ディストリビューションを削除した場合、Distributionとstatic用OACが新規作成できるので、これらのIDを指定せずにデプロイできます。

まず`terraform.tfvars`の`static_oac_id`と`web_acl_id`を空文字にします
...
デプロイ後にAWSリソースとは直接関係のない必要作業（DB設定や外部データのインポート等）を手作業で行います。順を追って手順説明します。

##### 踏み台インスタンスによるRDS操作

踏み台インスタンスでpsqlコマンドを用いて以下操作を実行します

- migratorユーザのパスワード変更
- PostGIS有効化
- appユーザーの作成と権限設定

まず以下コマンドで踏み台インスタンスを起動し（SSM有効化まで数分かかります）

```zsh
make maintenance-up
```

以下コマンドでSSM接続します（プロジェクトのルートフォルダに移動してから実行してください）

```zsh
aws ssm start-session --target $(cd terraform/maintenance && AWS_PROFILE=terraform terraform output -raw instance_id)
```
...
以下コマンドでpsqlからmigratorユーザでRDSに接続します（`{プロジェクト名}-db.xxxxxxxxxxxx.ap-northeast-1.rds.amazonaws.com`の部分はRDS → Databases → 当該インスタンスを選択して見られるRDSの接続用アドレスで置き換えてください。パスワードは`PLACEHOLDER_CHANGE_ME`のはずです）
...
以下コマンドでアプリ用ユーザー作成と各種権限設定を実施します。
...
ここまで完了したらSSMからログアウトし、以下コマンドで踏み台インスタンスを削除します
...
以下のフォルダ構成でデータが設置されている前提でアップロードしていきます（`v1.0-mini`の部分はデータセットに応じて適宜変更）
...
以下コマンドでS3にnuScenes形式データをアップロード（プロジェクトのルートフォルダで実行してください）
...
以下コマンドでECRにイメージプッシュ（Macの場合Docker Desktopを起動してから実行してください）
...
コンソールからECS → Clusters → {プロジェクト名}-clusterの当該Taskをクリック → Containers → migrationのStatusが`Stopped | Exit code: 0`となっていれば成功

失敗していたら、{プロジェクト名}-cluster → TasksのLast statusに理由が書いてあるのと、CloudWatch → Log Managementの該当ログ部分を見ると良いでしょう。

##### nuScenes本体＆Map expansionのDBインポートタスク実行

まず以下コマンドで`.env.make`の値を環境変数として読み込みます（Makefileを使用しないためコマンドで読み込む必要がある）

```zsh
source <(grep -v '^#' .env.make | sed 's/^/export /')
```

以下コマンドでECR用のVPCエンドポイント（com.amazonaws.ap-northeast-1.ecr.dkrとcom.amazonaws.ap-northeast-1.ecr.api）を起動します。

```zsh
cd terraform/deploy && AWS_PROFILE=terraform terraform apply -auto-approve \
  -var="region=${REGION}" \
...
aws ecs run-task \
  ...
  --task-definition ${PROJECT_NAME}-import-task \
  --launch-type FARGATE \
...
タスクが完了（ECSコンソールでExit code: 0を確認）したら、以下コマンドでMap expansionのDBインポート用のECSタスクを実行します

```zsh
aws ecs run-task \
  --cluster ${PROJECT_NAME}-cluster \
  --task-definition ${PROJECT_NAME}-import-map-task \
  --launch-type FARGATE \
  --network-configuration "awsvpcConfiguration={subnets=[${PRIVATE_SUBNET_ID}],securityGroups=[${SG_ECS_ID}],assignPublicIp=DISABLED}" \
  --region ${REGION} \
  --no-cli-pager \
...
タスクが完了（ECSコンソールでExit code: 0を確認）したら、以下コマンドでECR用のVPCエンドポイントを削除します。
...
作成したCloudFrontディストリビューションをクリックし、Distribution domain nameをコピーする。
ブラウザに以下アドレスを入力して開く（`{distribution-domain}`には調べたDistribution domain nameを入力）
