...
export type EgoDirection = 'x+' | 'x-' | 'y+' | 'y-' | 'z+' | 'z-'
...
export function translateAnnotation(
    ...
    const base: [number, number, number] = [
        DIRECTION_VECTORS[direction][0] * multiplier,
...
export function rotateAnnotation(
    ...
    const angleRad = (clockwise ? -ROTATION_STEP : +ROTATION_STEP) * multiplier * Math.PI / 180
...
export function resizeAnnotation(
    ...
    newSize[axis] = Math.max(SIZE_MIN, oldVal + sign * SIZE_STEP * multiplier)
