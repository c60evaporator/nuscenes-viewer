...
/**
 * BEV 画面座標 (Y軸下向き) → センサー座標系の (x, y) に逆変換する
 * sensorToBevPixel の逆関数。Konva の node.x(), node.y() 等の画面座標を入力する。
 *
 * @param px   BEV Canvas/Konva の画面 x 座標
 * @param py   BEV Canvas/Konva の画面 y 座標 (上が0、下が大きい値)
...
export function bevPixelToSensor(
  ...
  const cy = view.height / 2
  return [
    (cy - py) / view.scale + view.offsetX,
    (px - cx) / view.scale + view.offsetY,
  ]
}
